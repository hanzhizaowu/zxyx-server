...
public class AESCrypt {
    private static Logger logger = LoggerFactory.getLogger(AESCrypt.class.getName());
    ...
    private static SecretKeySpec generateKey(final String password) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        //if(logger.isDebugEnabled()) {
            logger.info("generateKey bytes:" + bytes);
        //}
        digest.update(bytes, 0, bytes.length);
        //if(logger.isDebugEnabled()) {
            logger.info("generateKey update bytes:" + bytes);
        //}

        byte[] key = digest.digest();
        logger.info("SHA-256 key " + key);
    ...
    public static String decrypt(final String password, String base64EncodedCipherText)
        ...
        try {
            final SecretKeySpec key = generateKey(password);
            logger.info("base64EncodedCipherText:" + base64EncodedCipherText);
            byte[] decodedCipherText = Base64.getDecoder().decode(base64EncodedCipherText);
            logger.info("decodedCipherText:" + decodedCipherText);
            byte[] decryptedBytes = decrypt(key, ivBytes, decodedCipherText);
            logger.info("decryptedBytes 2:" + decryptedBytes);
